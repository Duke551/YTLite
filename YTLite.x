...
// Fix stuck loading spinner on iPad
// Recursively walk a view hierarchy and hide/stop any stuck activity indicators
...
// Fix stuck loading spinner on iPad — clear stuck spinners when playback is confirmed active
    ...
    if (state == 2) {
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            if ([view isKindOfClass:NSClassFromString(@"YTPlayerView")]) {
                ...
                // Clean spinners in the overlay view
                if (playerView.overlayView) {
                    hideStuckSpinnersInView(playerView.overlayView);
                }
                // Also walk YTPlayerView's own subviews
